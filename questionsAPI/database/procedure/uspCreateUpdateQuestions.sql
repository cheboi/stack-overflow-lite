...
@id varchar(50),  @title varchar(50), @description varchar(max), @user_email varchar(50), @answer_id varchar(50), @isDeleted bit, @date_asked datetime,
...
Else IF @StatementType = 'editQuestion'
    BEGIN
  UPDATE dbo.questionsTable SET  id=@id, title=@title, description=@description, user_email=@user_email, date_asked=@date_asked
    WHERE id=@id
    END
END